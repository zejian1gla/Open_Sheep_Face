...
from pi_ert.base import Regressor, RegressorBuilder
from pi_ert.util import *
from pi_ert.get_pixel_value import GetPixelValueBuild
from pi_ert.tree import TreeBuilder
from pi_ert.forest import ForestBuilder
...
class CascadeForestBuilder(RegressorBuilder):
    ...
    # builds cascade forest regressor by iterating through a number of forests
    # and builds a new one with each iteration
    # then updates the shape estimates using the new forest
    def build(self, images, gt_shapes, boxes):
        self.get_pixel_value_builder = GetPixelValueBuild(n_landmarks=self.n_landmarks, n_pixels=self.n_pixels,
                                                          kappa=self.kappa)
        ...
        for i in range(self.n_forests):
            ...
            targets = np.array([transform_to_mean_shape(current_shapes[i],
                                                        self.mean_shape).apply(shape_residual[i]).reshape(
...
class CascadeForest(Regressor):
    ...
    # applies the cascade forest regressor to an image
    # takes an image and a list of bounding boxes as input
    # returns the initial shape estimates and the final shape estimates
